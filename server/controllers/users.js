...
export const signIn = async (req,res) => {
    ...
    try{
        ...
        if (!existingUser) return res.status.json({ message: "User does not exist" });
...
export const signUp = async (req,res) => {
    ...
    try{
        const existingUser = await User.findOne({ email });
        if (existingUser) return res.status.json({ message: "Account with that email already exists" });
